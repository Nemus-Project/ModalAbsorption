...
% modalRIR.m
%==========================================================================
% Modal fitting for room impulse responses via spectral peak-picking
% with independent per-mode damping estimation.
%
% TWO-REGIME STRATEGY
%   f < fTransition  (below Schroeder frequency)  — spectral peak-picking
%     with per-mode damping from the half-power (-3 dB) bandwidth.  This
%     is an independent damping estimate that does NOT use the Schroeder
%     integral, T20, or T30.  When the -3 dB crossing is blocked by a
%     neighbouring peak or band edge, a local Lorentzian LS fit is used.
%
...
%           Peak-picking runs on these; small bandwidths keep the number
%           of peaks per band manageable regardless of bandMode.
%         • Display bands — exact ISO 266 octave or third-octave bands.
%           All statistics, console output, and figures use these.
%   6.  Run peak-picking per processing band: half-power bandwidth damping
%       for LF, Schroeder T30 damping for HF.
...
%   acoustic_indices.m               (companion file in the same folder)
%
...
%   Peak-picking runs on narrow linear bands of this fixed width [Hz].
...
%   Below fTransition: LF peak-picking with half-power bandwidth damping.
%   Above fTransition: HF peak-picking with Schroeder T30 damping.
...
% --- LF peak-picking with half-power bandwidth damping --------------------
%   All LF modes are detected via spectral peak-picking.  Damping is
%   estimated from the -3 dB bandwidth of each resonance — an independent
%   measurement that does NOT use Schroeder / T20 / T30.  When the -3 dB
%   crossing is blocked by a neighbouring peak, a Lorentzian LS fit is
%   used instead.
LFpeakProm_dB      = 4.0;     % [dB]  minimum peak prominence for LF peaks
...
enableGainOutlierRejection = false;
gainOutlierMethod          = 'mad';    % 'mad' | 'std'
gainOutlierZthresh         = 4.0;     % (was 3.0)
maxGainOutlierLSIters      = 5;
minModesToKeepAfterReject  = 2;     % (was 3)

% --- Iterative residual refinement ----------------------------------------
useIterativeRefinement = false;    % (was false)
nRefinementIters       = 2;
mergeTol_Hz            = 0.15;    % [Hz]  (was 0.25)
...
% --- Acoustic indices -----------------------------------------------------
MinSecondsForFFT  = 15;      % minimum FFT length [s]  (was 5)
bandsPerOctave    = 3;
freqLimsAI        = [20, fHigh];
...
%   Peak-picking runs on these narrow bands.  Their width is
...
%% =================== INITIAL PEAK-PICKING PASS (per processing band) =====

fprintf('\n=== INITIAL PEAK-PICKING PASS (%d processing bands) ===\n', NProcBands);
...
for ib = 1 : NProcBands
    ...
    if strcmp(regime, 'LF')
        if verbose
            fprintf(['  Band %2d/%2d  [%6.1f – %6.1f Hz]  LF  |  ' ...
                ...
                'peak-pick (half-power bandwidth damping)\n'], ...
                ib, NProcBands, flow, fhigh, fcBand, bwBand, Nbins);
        end
        [fBand, cBand] = peakPickLF_bandwidth(fftBand, fvBand, df, ...
            LFpeakProm_dB, LFpeakDist_Hz);
    ...
    % Capture the per-pole T60 spread BEFORE any homogenisation.
    T60perPole = 3*log(10) ./ max(cBand, 1e-10);
    T60std_raw = std(T60perPole);
    if isnan(T60std_raw), T60std_raw = 0; end

    % For HF bands, replace individual per-pole damping with the band mean
    % (they all share Schroeder T30 damping anyway).
    % For LF bands, KEEP per-mode dampings — each one is an independent
    % measurement from the half-power bandwidth.
    if strcmp(regime, 'HF')
        cMean    = mean(cBand);
        cBand(:) = cMean;
    end
    cMean = mean(cBand);   % for display only
...
if useIterativeRefinement && nRefinementIters > 0
    ...
    for iIter = 1 : nRefinementIters
        ...
        for ib = 1 : NProcBands
            ...
            if strcmp(regime, 'LF')
                [fNew, cNew] = peakPickLF_bandwidth(fftResid, fvBand, df, ...
                    LFpeakProm_dB, LFpeakDist_Hz);
            else
                cFromT60 = 3*log(10) / max(T60est, 0.01);
            ...
            % For HF bands, pull new damping toward existing band mean.
            % For LF bands, keep the per-mode bandwidth damping as-is.
            regime_ib = selectRegime(procCentres(ib), fTransition);
            if strcmp(regime_ib, 'HF')
        ...
        % Re-average damping per processing band after adding poles (HF only).
        % LF bands retain per-mode half-power bandwidth damping.
        for ib = 1 : NProcBands
            idx = bandMembership{ib};
            if ~isempty(idx)
                regime_ib = selectRegime(procCentres(ib), fTransition);
                if strcmp(regime_ib, 'HF')
                    cAll(idx) = mean(cAll(idx));
                end
            end
        end
...
hFig_spectrum = figure('Name', 'modalRIR — Spectrum', ...
...
% --- Per-band statistics --------------------------------------------------
hFig_bandStats = figure('Name', 'modalRIR — Per-band statistics', ...
    'Color', 'w', 'Position', [50 800 1200 600]);
plotBandStats(poleStats, globalStats, NDispBands, fTransition);

% --- LF T60 comparison: modal poles vs acoustic_indices -----------------
hFig_LFcomp = figure('Name', 'modalRIR — LF T60 comparison', ...
...
savePath = fullfile(outSubDir, [filename, '_modalRIR.mat']);
save(savePath, '-struct', 'saveS');
fprintf('MAT saved   : %s\n', savePath);

outSynNorm = outSyn / (max(abs(outSyn)) + eps);
audiowrite(fullfile(outSubDir, [filename, '_modalRIR_SYNTH.wav']), outSynNorm, fs);
...
%%    2.  Parameter selection   (interpT60band)
%%    3.  Pole estimation       (peakPickWrapper, peakPickLF_bandwidth)
...
function [fPeaks, cPeaks] = peakPickLF_bandwidth(fftBand, fvBand, df, ...
    minProm_dB, minDist_Hz)
%PEAKPICKLF_BANDWIDTH  LF peak detection with half-power bandwidth damping.
%
%   Detects peaks in |H(f)| and estimates per-mode damping from the
%   -3 dB bandwidth of each resonance.  This is an INDEPENDENT damping
%   measurement that does not use the Schroeder integral, T20, or T30.
%
%   For a lightly damped single-degree-of-freedom resonance the half-power
%   bandwidth relates to the exponential decay rate as:
%
%       Δf_3dB  =  σ / π       =>     σ  =  π · Δf_3dB      [rad/s]
%       T60     =  3·ln(10) / σ
%
%   When the -3 dB level cannot be reached cleanly (neighbouring peak
%   intervenes or band edge is hit), a local Lorentzian least-squares fit
%   is used instead:
%
%       |H(f)|² ≈ A / [ (f - f₀)² + γ² ]       with   σ = 2π γ
%
%   INPUTS
%     fftBand    : complex single-sided FFT of the band [Nbins × 1]
%     fvBand     : frequency vector [Hz]
%     df         : frequency resolution [Hz]
%     minProm_dB : minimum peak prominence [dB]
%     minDist_Hz : minimum inter-peak spacing [Hz]
%
%   OUTPUTS
%     fPeaks     : peak frequencies [Hz]
%     cPeaks     : damping coefficients [rad/s]   (T60 = 3·ln10 / c)
...
% --- Per-peak damping from -3 dB bandwidth --------------------------------
Npeaks = numel(locs);
fPeaks = zeros(Npeaks, 1);
cPeaks = zeros(Npeaks, 1);

for k = 1 : Npeaks
    pk         = locs(k);
    fPeaks(k)  = fvBand(pk);
    peakVal_dB = dB_band(pk);
    thresh_dB  = peakVal_dB - 3.0;

    % -- Search LEFT for the -3 dB crossing --------------------------------
    iLeft      = 1;
    leftClean  = false;
    for ii = pk - 1 : -1 : 1
        if dB_band(ii) <= thresh_dB
            % Sub-bin interpolation
            span   = dB_band(ii+1) - dB_band(ii);
            frac   = (thresh_dB - dB_band(ii)) / (span + eps);
            frac   = max(0, min(1, frac));
            iLeft  = ii + frac;
            leftClean = true;
            break;
        end
        % Stop at a valley (local minimum that is clearly below the peak)
        if ii > 1 && dB_band(ii) < dB_band(ii-1) && ...
                     dB_band(ii) < dB_band(ii+1) && ...
                     dB_band(ii) < peakVal_dB - 1.0
            iLeft = ii;
            break;
        end
    end
    ...
    for ii = pk + 1 : Nb
        ...
        if ii < Nb && dB_band(ii) < dB_band(ii-1) && ...
                      dB_band(ii) < dB_band(ii+1) && ...
                      dB_band(ii) < peakVal_dB - 1.0
            iRight = ii;
            break;
    ...
    fLeft  = interp1(1:Nb, fvBand, max(1, iLeft),  'linear', 'extrap');
    fRight = interp1(1:Nb, fvBand, min(Nb, iRight), 'linear', 'extrap');

    if leftClean && rightClean
        % Both -3 dB points found cleanly → standard half-power bandwidth
        deltaf     = max(fRight - fLeft, df);
        cPeaks(k)  = pi * deltaf;

    else
        % At least one side was truncated by a valley or band edge.
        % Fall back to a local Lorentzian LS fit:
        %     |H(f)|² ≈  A / [ (f - f0)² + gamma² ]
        %     σ = 2π · gamma
        fitRadius  = max(round(0.15 * Nb), 5);
        i1         = max(1,  pk - fitRadius);
        i2         = min(Nb, pk + fitRadius);
        fLocal     = fvBand(i1:i2) - fvBand(pk);      % centred at peak
        magSq      = magBand(i1:i2).^2;
        magSq      = magSq / max(magSq);               % normalise

        % Solve  1/magSq ≈ (fLocal² + gamma²) / A
        %   i.e. 1/magSq = (1/A)*fLocal² + gamma²/A
        %   Let  x = [1/A;  gamma²/A]
        invMagSq   = 1 ./ max(magSq, 1e-12);
        G          = [fLocal.^2, ones(numel(fLocal), 1)];
        x          = G \ invMagSq;

        if x(1) > 0 && x(2) > 0
            gamma      = sqrt(x(2) / x(1));          % Hz
            cPeaks(k)  = 2*pi * gamma;                % rad/s
        else
            % Last resort: use whatever partial bandwidth we got
            deltaf     = max(fRight - fLeft, df);
            cPeaks(k)  = pi * deltaf;
        end
    end
...
% --- Sanity filter --------------------------------------------------------
%   Reject peaks with implausibly short or long T60.
T60vec = 3*log(10) ./ max(cPeaks, eps);
keep   = (T60vec > 0.05) & (T60vec < 60);
fPeaks = fPeaks(keep);
cPeaks = cPeaks(keep);
end
...
% --- Extract Modal T60 statistics for LF bands -------------------------
...
% Open markers for LF bands where Modal found 0 poles
...
% Annotate mode count above each Modal marker
for k = 1 : NLF
    if pm_N(k) > 0
...
grid on;
title(sprintf(['LF bands (f_c < %.0f Hz) — Modal T_{60} vs acoustic-indices T_{30}  (ISO 3382)\n' ...
